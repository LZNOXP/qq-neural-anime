...
export const qqRequest = async (imgData: string) => {
		...
		extra = await asyncRetry(
			async (bail) => {
				...
				if (
					...
					bail(
						new Error("The Chinese website has blocked the bot, too bad 🤷‍♂️")
					);
					return;
				}
			...
			},
			{
				onRetry(e, attempt) {
					console.error(
						`QQ file upload error caught (attempt #${attempt}): ${e.toString()}`
					);
				},
				retries: 10,
				factor: 1,
	...
	return {
		...
		singleImgUrl:
			QQ_MODE === "CHINA" ? (extra.img_urls[2] as string) : undefined,
	};
};
	...
	try {
		data = await asyncRetry(
			...
			},
			{
				onRetry(e, attempt) {
					console.error(
						`QQ file download error caught (attempt #${attempt}): ${e.toString()}`
					);
				},
				retries: 10,
				factor: 1,
...
export const qqCropImage = async (
	imgData: Buffer,
	type: "COMPARED" | "SINGLE"
): Promise<Buffer> => {
